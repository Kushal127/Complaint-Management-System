...
/* Class.forName(driver);
 ...
 PreparedStatement st2=con.prepareStatement("select max(cid) from CMS_CUSTOMER ");
 ...
 if(rs.next())
  ...
  PreparedStatement st1=con.prepareStatement("insert  into login  values('"+lid+"','"+pwd1+"',"+roleid+",'"+status+"',"+cid1+")");
   int k=st1.executeUpdate();
	  System.out.println("hello in ras22"+k);

   PreparedStatement st4=con.prepareStatement("insert  into CMS_CUSTOMER  values('"+lid+"','"+pwd1+"', '"+cname+"','"+addr+"','"+email+"', '"+phno+"','"+accno+"',"+atmno+",'"+pin+"','"+otno+"','"+opin+"','"+status+"',"+cid1+")");
  ...
  PreparedStatement st3=con.prepareStatement("insert  into cms_service  values('"+accno+"','"+atm+"','"+bank+"','"+online+"',"+cid1+")");
